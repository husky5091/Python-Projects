...
def main():
    try:
        print('+-+-+-+-++-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+'), print('|             PyMaths CMD (commands)               |'),       print('|--------------------------------------------------|'),        print('| a) addition             | k) cos (cosine)        |'),         print('| b) substraction         | l) tan (tangent)       |'),         print('| c) multiply             | m) absolute value      |'), print('| d) division             | n) exponentiation      |'), print('| e) square root          | o) logarithm10 (log10) |'),print('| f) modulos              | p) logarithm (log)     |'),print('| g) odd or even          | q) degrees to radians  |'),print('| h) pozitive or negative | r) radians to degrees  |'),print('| i) random number        | s) prime number        |'),print('| j) sin (sine)           | t) logarithm2 (log2)   |'),print('| px) PyMaths details     | u) ceil                |'),print('+-+-+-+-++-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+'),print('PyMaths started at', datetime.now(), '!'),print('Entering an invalid operation will restart the script.'),print("WARNING! Float numbers are written with '.' not with ',' !")
        usr_choice = input(">> ")
